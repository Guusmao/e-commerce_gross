...
const InputField = ({ label, id, type = 'text', value, onChange, placeholder, required = true, disabled = false }) => (
  <div>
    ...
    <input
      ...
      className={`mt-1 block w-full px-3 py-2 bg-gray-50 border border-gray-300 rounded-md shadow-sm placeholder-gray-400 focus:outline-none focus:ring-primary focus:border-primary sm:text-sm ${disabled ? 'cursor-not-allowed opacity-70' : ''}`}
    />
  </div>
);
...
const CompletarCadastro = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // O backend espera firstname/surname separados; o front só coleta o nome completo,
    // então dividimos no primeiro espaço (ajustar se um dia o formulário separar os campos).
    const [firstname, ...resto] = formData.nome.trim().split(' ');
    const surname = resto.join(' ') || firstname;
    ...
    try {
      ...
      // Cadastro criado -> já faz login automático pra não pedir a senha de novo
      const loginResponse = await api.post('/usuario/token', {
        email: formData.email,
        password: formData.password,
      });
      ...
      const errorMessage = error.response?.data?.error || 'Não foi possível concluir o cadastro. Tente novamente.';
      alert(errorMessage);
    } finally {
        ...
        <form onSubmit={handleSubmit} className="bg-white p-8 rounded-lg shadow-md space-y-6">
          ...
          <div className="pt-4">
            <label className="flex items-center">
              <input type="checkbox" name="receberEmails" checked={formData.receberEmails} onChange={handleChange} className="h-4 w-4 text-primary border-gray-300 rounded focus:ring-primary" />
              <span className="ml-2 text-sm text-gray-600">Quero receber por email ofertas e novidades das lojas da Gross.</span>
            </label>
          </div>

          <button type="submit" disabled={isSubmitting} className="w-full bg-primary hover:brightness-110 disabled:opacity-60 text-white font-bold py-3 px-4 rounded-md transition-colors">
