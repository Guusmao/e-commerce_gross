    ...
    <div className="flex flex-col">
      ...
      */}
      ...
      <div className="flex items-baseline gap-2">
        <span className="text-2xl font-bold text-primary">{formatPrice(product.priceDiscount)}</span>
            ...
            {product.sizes.map(size => (
              <button key={size} onClick={() => setSelectedSize(size)}
                className={`w-11 h-11 flex items-center justify-center border rounded-md transition-all text-sm ${
                  selectedSize === size ? 'bg-primary text-white border-primary font-bold' : 'bg-white text-gray-700 border-gray-300 hover:border-gray-500'
                }`}>
                {size}
              </button>
            ))}
            ...
            {product.colors.map(color => (
              <button key={color} onClick={() => setSelectedColor(color)}
                className={`w-8 h-8 rounded-full border-2 transition-transform duration-150 transform ${
                  selectedColor === color ? 'border-primary scale-110' : 'border-gray-400'
                }`}
